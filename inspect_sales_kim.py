...
target_mgr = "김길래"
print(f"--- [{target_mgr}] 실적 상세 분석 (매출 기준) ---")
    ...
    if mgr_col in df.columns:
        ...
        if not matched.empty:
            ...
            for idx, row in matched.iterrows():
                ...
                # 연도별 컬럼이 0인 경우 월별 합산 시도 (안전 장치)
                if row_s == 0:
                    m_cols = [f"Deal - @월별매출 ({m:02d})" for m in range(1, 13)]
                    row_s = sum(clean_currency_val(row.get(c, 0)) for m in range(1, 13))
                
                reflected_s = row_s * ratio
                if row_s != 0:
                    print(f"- {row['Deal - 이름']}: 원매출 {row_s:,.0f}원 -> 반영매출 {reflected_s:,.1f}원")
                    total_actual_sales += reflected_s

print("\n" + "="*40)
print(f"최종 합계 계산 결과: {total_actual_sales:,.1f}원")
print("="*40)
